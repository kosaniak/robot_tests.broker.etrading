...
Library     Collections
Library     etrading_helper.py


*** Variables ***
${TESTDOMAIN}                                        http://test.e-trading.kiev.ua
${sign_in}                                           id=loginbtn
...
Підготувати клієнт для користувача
  ...
  Run Keyword If   '${username}' != 'etrading_viewer'   Login   ${username}
...
Створити тендер
  ...
  ...      ${ARGUMENTS[1]} ==  tender_data
    ...
    ${value_valueaddedtaxincluded}=          convert_etrading_string     ${value_valueaddedtaxincluded}
    ...
    ${auctionperiod_startdate}=        etrading_convertdate   ${auctionperiod_startdate}
    ...
    etrading.Додати предмети      ${items}
    Sleep   5
...
Додати предмети
    ...
    \   etrading.Додати предмет   ${items[${index}]}     ${index}
...
Додати предмет
    ...
    ${contractperiod_startdate}=        etrading_convertdate     ${item.contractPeriod.startDate}
    Input text      id=additemform-${index}-contractperiod_startdate   ${contractperiod_startdate}
    ${contractperiod_enddate}=        etrading_convertdate     ${item.contractPeriod.endDate}
...
Задати запитання на тендер
  ...
  etrading.Задати питання  ${username}  ${tender_uaid}  ${question}
...
Оновити сторінку з тендером
    ...
    ...      ${ARGUMENTS[1]} = ${TENDER_UAID}
    Switch Browser   ${BROWSER_ALIAS}
    etrading.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[1]}
...
Отримати інформацію про awards[0].status
    ...
    ${return_value}=   convert_etrading_string     ${return_value}
    [Return]  ${return_value}
...
Отримати інформацію про tenderPeriod.startDate
  ...
  ${return_value}=   convert_etrading_date_to_iso_format   ${return_value}
...
Отримати інформацію із документа
  ...
  ${tender}=  etrading.Пошук тендера по ідентифікатору  ${username}  ${tender_uaid}
...
Отримати документ
    ...
    etrading_download_file   ${url}  ${file_name}  ${OUTPUT_DIR}
    [return]  ${file_name}
...
Завантажити фінансову ліцензію
    [Arguments]  ${username}  ${tender_uaid}  ${filepath}
    etrading.Завантажити документ в ставку  ${username}  ${filepath}  ${tender_uaid}
...
Отримати інформацію із пропозиції
    ...
    ${bid}=   etrading.Отримати пропозицію  ${field}
...
Оновити сторінку з об'єктом МП
    ...
    etrading.Пошук об’єкта МП по ідентифікатору  ${username}  ${tender_uaid}
...
Отримати інформацію із об'єкта МП
    ...
    etrading.Пошук об’єкта МП по ідентифікатору  ${username}  ${tender_uaid}
    Run Keyword If
    ...    'documentType' not in '${fieldname}'  etrading.wait with reload    assetlocator   ${fieldname}
    ${return_value}=   Get Text  ${assetlocator.${fieldname}}

    ${return_value}=  Run Keyword If
    ...  'status' in '${fieldname}'                                   convert_etrading_string  ${return_value}
    ...  ELSE IF    'registrationDetails.status' in '${fieldname}'    convert_etrading_string  ${return_value}
...
Створити лот
    ...
    etrading.Пошук об’єкта МП по ідентифікатору  ${username}  ${asset_uaid}
...
Внести зміни в умови проведення аукціону
    ...
    ...  'startDate' in '${fieldname}'    etrading_convertdate  ${fieldvalue}
